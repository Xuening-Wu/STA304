...
As can be seen from the table, our models predicted that there will be `r round(census_table1$liberal_predict * 100, 2)`% voters voting for the Liberal Party, `r round(census_table1$Conservative_predict * 100, 2)`% voting for the Conservative Party, and `r round(census_table1$ndp_predict * 100, 2)`% voting for the NDP. Together, the three major political parties take approximately `r round((census_table1$liberal_predict + census_table1$Conservative_predict + census_table1$ndp_predict) *100, 2)`% of the total votes, suggesting little likelihood that other parties will have votes higher than any of the three. Therefore, the prediction indicates that the Liberal Party is the most likely party to win the election because it has the largest predicted proportion of voters voting for them. 
...
```{r echo=FALSE}
kable(glimpse(census_data_counts))
```
