...
The 3 binary response variables denote whether an individual voted for the that corresponding party (1 for yes, 0 for no).
...
Since we have 3 regression models, we will get the proportion of voting the 3 parties by Post-Stratification. The party with the highest proportion will be predicted to win the election. 
...
report(mod2)
```
Since the models for predicting all 3 major political parties will be the same, for simplicity, we will showcase our model selection process using the model for Liberal party only. 
...
I used the following generative artificial intelligence (AI) tool: ChatGPT [4]. 
